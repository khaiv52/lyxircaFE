  ...
  useEffect(() => {
    if (
      ...
      const fetchProducts = async () => {
        try {
          // Tìm cấp 1
          const getLevelOneCategory = (data, levelOneName) => {
            return data.find((category) => category.name === levelOneName);
          };

          // // Tìm cấp 2
          const getLevelTwoCategory = (levelOneCategory, levelTwoName) => {
            if (!levelOneCategory || !levelOneCategory.sections) return null;
            return levelOneCategory.sections.find(
              (section) => section.name === levelTwoName
            );
          };

          // Tìm cấp 3
          const getLevelThreeCategory = (levelTwoCategory, levelThreeName) => {
            if (!levelTwoCategory || !levelTwoCategory.categoryItems)
              return null;
            return levelTwoCategory.categoryItems.find(
              (item) => item.name === levelThreeName
            );
          };

          const levelOneCategory = getLevelOneCategory(
            categories,
            param.levelOne
          );

          const levelTwoCategory = getLevelTwoCategory(
            levelOneCategory,
            param.levelTwo
          );
          const levelThreeCategory = getLevelThreeCategory(
            levelTwoCategory,
            param.levelThree
          );

          // console.log("levelOneCategory:", levelOneCategory);
          // console.log("levelTwoCategory:", levelTwoCategory);
          // console.log("levelThreeCategory:", levelThreeCategory);

          // Kiểm tra levelOne từ URL có khớp với levelOne tìm được không
          if (
            levelOneCategory &&
            levelOneCategory.name === param.levelOne &&
            levelTwoCategory &&
            levelTwoCategory.name === param.levelTwo &&
            levelThreeCategory &&
            levelThreeCategory.name === param.levelThree
          ) {
            ...
            const data = {
              parentCategoryName: levelTwoCategory
                ? levelTwoCategory.name
                : null,
              category: levelThreeCategory ? levelThreeCategory.name : null,
