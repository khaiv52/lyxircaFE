export const FIND_PRODUCT_BY_ID_REQUEST = 'FIND_PRODUCT_BY_ID_REQUEST';
export const FIND_PRODUCT_BY_ID_SUCCESS = 'FIND_PRODUCT_BY_ID_SUCCESS';
export const FIND_PRODUCT_BY_ID_FAILURE = 'FIND_PRODUCT_BY_ID_FAILURE';

export const FIND_PRODUCTS_REQUEST = "FIND_PRODUCTS_REQUEST";
export const FIND_PRODUCTS_SUCCESS = "FIND_PRODUCTS_SUCCESS";
export const FIND_PRODUCTS_FAILURE = "FIND_PRODUCTS_FAILURE";

export const CREATE_PRODUCTS_REQUEST = "CREATE_PRODUCTS_REQUEST";
export const CREATE_PRODUCTS_SUCCESS = "CREATE_PRODUCTS_SUCCESS";
export const CREATE_PRODUCTS_FAILURE = "CREATE_PRODUCTS_FAILURE";

export const UPDATE_PRODUCTS_REQUEST = "UPDATE_PRODUCTS_REQUEST";
export const UPDATE_PRODUCTS_SUCCESS = "UPDATE_PRODUCTS_SUCCESS";
export const UPDATE_PRODUCTS_FAILURE = "UPDATE_PRODUCTS_FAILURE";

export const DELETE_PRODUCTS_REQUEST = "DELETE_PRODUCTS_REQUEST";
export const DELETE_PRODUCTS_SUCCESS = "DELETE_PRODUCTS_SUCCESS";
export const DELETE_PRODUCTS_FAILURE = "DELETE_PRODUCTS_FAILURE";

export const GET_PRODUCTS_BY_CATEGORY_NAME_REQUEST = "GET_PRODUCTS_BY_CATEGORY_NAME_REQUEST";
export const GET_PRODUCTS_BY_CATEGORY_NAME_SUCCESS = "GET_PRODUCTS_BY_CATEGORY_NAME_SUCCESS";
export const GET_PRODUCTS_BY_CATEGORY_NAME_FAILURE = "GET_PRODUCTS_BY_CATEGORY_NAME_FAILURE";

export const CLEAR_PRODUCTS_BY_CATEGORY_REQUEST = "CLEAR_PRODUCTS_BY_CATEGORY_REQUEST";
export const CLEAR_PRODUCTS_BY_CATEGORY_SUCCESS = "CLEAR_PRODUCTS_BY_CATEGORY_SUCCESS";
export const CLEAR_PRODUCTS_BY_CATEGORY_FAILURE = "CLEAR_PRODUCTS_BY_CATEGORY_FAILURE";

export const FIND_PRODUCTS_BY_SEARCH_TEXT_REQUEST = "FIND_PRODUCTS_BY_SEARCH_TEXT_REQUEST";
export const FIND_PRODUCTS_BY_SEARCH_TEXT_SUCCESS = "FIND_PRODUCTS_BY_SEARCH_TEXT_SUCCESS";
export const FIND_PRODUCTS_BY_SEARCH_TEXT_FAILURE = "FIND_PRODUCTS_BY_SEARCH_TEXT_FAILURE";

CREATE_CATEGORY_REQUEST, 
CREATE_CATEGORY_SUCCESS, CREATE_CATEGORY_FAILURE